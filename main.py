...
class BlogHandler(webapp2.RequestHandler):
    ...
    def get_posts_by_user(self, user, limit, offset):
        ...
        query = Post.all()
        query.filter("author", user).order('-created')
        return query.fetch(limit=limit, offset=offset)
...
class SignupHandler(BlogHandler):
    ...
    def post(self):
        ...
        if has_error:
            t = jinja_env.get_template("signup.html")
            response = t.render(username=username, email=email, errors=errors)#changed error to errors
            self.response.out.write(response)
        else:
...
class LoginHandler(BlogHandler):
    ...
    def render_login_form(self, username ="", password ="", errors=""):#added username & pword & changed error to errors
        """ Render the login form with or without an error, based on parameters """
        t = jinja_env.get_template("login.html")
        response = t.render(username = username, password = password, errors=errors)#added username & pword & changed error to errors
        self.response.out.write(response)
    ...
    def post(self):
        ...
        if not user:
            self.render_login_form(errors="Invalid username")#changed to errors from error
        elif hashutils.valid_pw(submitted_username, submitted_password, user.pw_hash):
            self.login_user(user)
            self.redirect('/blog/newpost')
        else:
            self.render_login_form(errors="Invalid password")#changed to errors from error
